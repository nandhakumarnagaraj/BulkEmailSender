...
import com.bulkemail.service.EmailService;
...
public class BulkEmailController {
  ...
  private static final byte[] PIXEL_BYTES = new byte[] {
      ...
      0x00, 0x01, 0x0D, 0x0A, 0x2D, (byte) 0xB4, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, (byte) 0xAE, 0x42,
      0x60, (byte) 0x82
  };

  @PostMapping("/api/send")
  public ResponseEntity<String> sendBulkEmail(@RequestBody BulkEmailRequest request) {
    emailService.sendBulkEmail(request.getRecipients(), request.getSubject(), request.getBody());
    return ResponseEntity.ok("Bulk email process started for " + request.getRecipients().size() + " recipients.");
  }
